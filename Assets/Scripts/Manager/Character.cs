...
namespace Manager
{
    public class Character : MonoBehaviour
    {
        ...
        {
            RectTransform transform = Spawn(position, Prefabs.Player, parent);
        ...
        {
            RectTransform transform = Spawn(position, Prefabs.NPC, parent);
            ...
            worldCharacter.SetCharacter(character, Sprites.GetCharacter(character.GetSpriteName()));
        }

        public void SpawnEnemy(Vector3 position, Enemies enemyType, Transform parent)
        {
            RectTransform transform = Spawn(position, Prefabs.Enemy, parent);

            World.Enemy worldCharacter = transform.GetComponent<World.Enemy>();
            Enemy character = new Enemy { characterType = enemyType };
            worldCharacter.SetCharacter(character, Sprites.GetEnemy(character.GetSpriteName()));
        }
    }
}
